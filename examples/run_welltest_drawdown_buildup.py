...
# keep your tweak
k_md_3d[1, 10, 14] = 76.99486   # (all k, j=10, i=14)
...
# ============================================================
# WELL TEST (Drawdown 72 hr -> Shut-in 55 hr, aquifer OFF during buildup)
# Exports Excel: WellTest_Drawdown_Buildup.xlsx (two sheets)
# ============================================================
from pathlib import Path

WELLTEST_DRAW_HR = 72.0
WELLTEST_BUILD_HR = 55.0

DT_DRAW_HR = 1.0  # drawdown output step (hr)

def dt_buildup_hr(t_shut_hr: float) -> float:
    # finer early time to capture curvature
    if t_shut_hr < 1.0:
        return 0.05
    if t_shut_hr < 6.0:
        return 0.10
...
def export_welltest_to_excel(
    xlsx_path: Path,
    t_draw_hr: np.ndarray,
    pwf_psi: np.ndarray,
    t_bu_hr: np.ndarray,
    pws_psi: np.ndarray,
) -> None:
    ...
    ws = wb.active
    ws.title = "Drawdown_Pwf"
    ws.append(["t_draw_hr", "Pwf_psi"])
    for t, p in zip(t_draw_hr.tolist(), pwf_psi.tolist()):
        ws.append([float(t), float(p)])

    ws = wb.create_sheet("Buildup_Pws")
    ws.append(["t_shut_hr", "Pws_psi"])
    for t, p in zip(t_bu_hr.tolist(), pws_psi.tolist()):
        ws.append([float(t), float(p)])

    wb.save(xlsx_path)


def run_welltest_drawdown_then_buildup():
    global dt, BC_BOTTOM, AQUIFER_BOTTOM_MULT

    if len(prod_wells) < 1:
        raise RuntimeError("prod_wells is empty. Add one producer first.")

    # ---- Save original settings so main() still works later ----
    orig_dt = dt
    orig_BC_BOTTOM = BC_BOTTOM
    orig_AQ = AQUIFER_BOTTOM_MULT
    orig_q = float(prod_wells[0].get("q_total", 0.0))

    # Initial state
    P = np.full((Nz, Ny, Nx), P0, dtype=float)
    Sw = np.full((Nz, Ny, Nx), Sw0, dtype=float)

    # Keep saturation constant for the test (well-test style)
    lam_w, lam_o, lam_t, fw = mobilities(Sw, Swi, Sor, mu_w, mu_o, Bw, Bo)

    w = prod_wells[0]  # use your PRD1

    try:
        # =====================================================
        # 1) DRAWDOWN: produce for 72 hr (aquifer stays as you set it)
        # =====================================================
        w["q_total"] = orig_q  # keep your 300 STB/d (or whatever you set in prod_wells)

        t_draw = [0.0]
        pwf_draw = [float(compute_producer_pwf(P, lam_t, w))]

        t_hr = 0.0
        while t_hr < WELLTEST_DRAW_HR - 1e-12:
            dt_hr = min(DT_DRAW_HR, WELLTEST_DRAW_HR - t_hr)
            dt = dt_hr / 24.0  # IMPORTANT: your solver uses dt in DAYS
            t_hr += dt_hr

            P, Tx, Ty, Tz = pressure_solve_3d(P, Sw, lam_t)
            pwf = float(compute_producer_pwf(P, lam_t, w))

            t_draw.append(t_hr)
            pwf_draw.append(pwf)

        t_draw_arr = np.array(t_draw, dtype=float)
        pwf_draw_arr = np.array(pwf_draw, dtype=float)

        # ---- Plot drawdown ----
        plt.figure()
        plt.plot(t_draw_arr, pwf_draw_arr, linewidth=2)
        plt.xlabel("Elapsed time (hr)")
        plt.ylabel("Flowing pressure Pwf (psi)")
        plt.title("Drawdown: Pwf vs time")
        plt.grid(True)
        plt.show()

        # =====================================================
        # 2) SHUT-IN BUILDUP: 55 hr
        #    shut off EVERYTHING: q=0 AND aquifer OFF
        # =====================================================
        w["q_total"] = 0.0
        BC_BOTTOM = False
        AQUIFER_BOTTOM_MULT = 0.0

        t_bu = [0.0]
        pws_bu = [float(compute_producer_pwf(P, lam_t, w))]  # with q=0 -> Pws

        t_shut = 0.0
        while t_shut < WELLTEST_BUILD_HR - 1e-12:
            dt_hr = min(dt_buildup_hr(t_shut), WELLTEST_BUILD_HR - t_shut)
            dt = dt_hr / 24.0
            t_shut += dt_hr

            P, Tx, Ty, Tz = pressure_solve_3d(P, Sw, lam_t)
            pws = float(compute_producer_pwf(P, lam_t, w))

            t_bu.append(t_shut)
            pws_bu.append(pws)

        t_bu_arr = np.array(t_bu, dtype=float)
        pws_bu_arr = np.array(pws_bu, dtype=float)

        # ---- Plot buildup ----
        plt.figure()
        plt.plot(t_bu_arr, pws_bu_arr, linewidth=2)
        plt.xlabel("Elapsed shut-in time (hr)")
        plt.ylabel("Shut-in pressure Pws (psi)")
        plt.title("Buildup: Pws vs time (Aquifer OFF during buildup)")
        plt.grid(True)
        plt.show()

        # =====================================================
        # 3) EXCEL export (Drawdown + Buildup)
        # =====================================================
        out_xlsx = Path(__file__).with_name("WellTest_Drawdown_Buildup.xlsx")
        export_welltest_to_excel(out_xlsx, t_draw_arr, pwf_draw_arr, t_bu_arr, pws_bu_arr)
        print(f"\n[WellTest] Excel saved: {out_xlsx}")

        # Optional: print arrays to copy/paste
        print("\nDrawdown arrays:")
        print("t_draw_hr =", t_draw_arr)
        print("pwf_psi   =", pwf_draw_arr)

        print("\nBuildup arrays:")
        print("t_bu_hr   =", t_bu_arr)
        print("pws_psi   =", pws_bu_arr)

    finally:
        # ---- restore engine settings ----
        dt = orig_dt
        BC_BOTTOM = orig_BC_BOTTOM
        AQUIFER_BOTTOM_MULT = orig_AQ
        w["q_total"] = orig_q
...
if __name__ == "__main__":
    run_welltest_drawdown_then_buildup()
